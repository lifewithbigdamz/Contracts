...
impl VestingContract {
    ...
    fn calculate_time_vested_amount_at(vault: &Vault, timestamp: u64) -> i128 {
        if timestamp < vault.start_time { return 0; }
        if timestamp >= vault.end_time { return vault.total_amount; }
        
        let duration = vault.end_time - vault.start_time;
        if duration == 0 { return vault.total_amount; }
        
        let elapsed = timestamp - vault.start_time;
    ...
    pub fn batch_revoke(env: Env, vault_ids: Vec<u64>) -> i128 {
        ...
        let timestamp = env.ledger().timestamp();
        env.events().publish(
            (Symbol::new(&env, "BatchRevoked"), vault_ids.len() as u64),
    ...
    pub fn preview_claimable_at(env: Env, vault_id: u64, future_timestamp: u64) -> i128 {
        let vault: Vault = env
            .storage()
            .instance()
            .get(&DataKey::VaultData(vault_id))
        ...
        let unlocked_amount = if env.storage().instance().has(&DataKey::VaultMilestones(vault_id)) {
            // Future milestones can't be easily projected by time alone since they are manually unlocked.
            // We return the currently unlocked milestone percentage instead.
            let milestones = Self::require_milestones_configured(&env, vault_id);
            let unlocked_pct = Self::unlocked_percentage(&milestones);
            Self::unlocked_amount(vault.total_amount, unlocked_pct)
        } else {
            Self::calculate_time_vested_amount_at(&vault, future_timestamp)
        };

        let claimable = unlocked_amount - vault.released_amount;
        if claimable < 0 {
            0
        } else {
            claimable
